...
Phase 1 is COMPLETE. All 49 tests pass (43 lib + 2 creator + 4 runner).

### Achievements

- **Creator tests**: YAML vs programmatic equality comparison, compilation test
- **Runner tests**: Goal reachability, counter value checks, initial state verification
- **Infrastructure**: gen-fixture binary, test helper module, stubs system
- **Codegen fixes**: Type-aware value literals (value_to_literal_typed), Value deserialization

### File Structure

```
src/bin/creator/src/tests/
├── mod.rs                  — test module root
└── counter_test.rs         — YAML + programmatic builder + equality test + compilation test

src/bin/runner/src/tests/
├── mod.rs                  — test module root
├── helper.rs               — shared runtime test utilities (run_until, run_for)
└── counter_test.rs         — runner-specific tests (goal reachability, value checks)

src/bin/runner/generated/counter_test/
├── counter_test/
│   ├── types.rs            — generated types (Persistent, State)
│   └── tick.rs             — generated tick/init functions
├── group.rs                — generated group code
└── mod.rs                  — generated module declarations

src/bin/gen-fixture.rs       — binary to regenerate fixture files
```

### Running Tests

```bash
cargo test -p pall                # Run all tests
cargo test -p pall creator        # Run creator tests only
cargo test -p pall runner         # Run runner tests only
cargo run --bin gen-fixture       # Regenerate fixture files
```
