...
use pall::machine::{
    ...
    State, StateMachine, StringFmt, StringValue, Timer, Transition, Type, Value, Variable, IntegerFmt, IntegerValue,
};
...
fn main() {
    let machines = vec![
        ...
        // build_type_system() - deferred: literal signed->unsigned casting requires separate plan
    ];
...
// Helper functions for type_system test (alias names for clarity)
fn bool_var(val: bool) -> Variable {
    Variable {
        r#type: Type::Bool,
    ...
    let verify_state = State {
        actions: vec![Action {
            ...
            r#do: vec![
                ...
                FullStatement::parse(r#"str_val = "hello""#).unwrap(),
            ],
        }],
        transitions: vec![Transition {
            when: None,
